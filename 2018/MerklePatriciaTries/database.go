...
import (
	...
	"gitlab.quachain.net/aba/aba/common/elog"
	"gitlab.quachain.net/aba/aba/core/types"
)

var log = elog.NewLogger("trie", elog.DebugLog)
...
func (db *Database) insert(hash types.Hash, blob []byte) {
	...
	db.nodesSize += types.StorageSize(types.HashLen + len(blob))
}
...
func (db *Database) insertPreimage(hash types.Hash, preimage []byte) {
	...
	db.preimagesSize += types.StorageSize(types.HashLen + len(preimage))
}
...
func (db *Database) dereference(child types.Hash, parent types.Hash) {
	...
	if node.parents == 0 {
		...
		db.nodesSize -= types.StorageSize(types.HashLen + len(node.blob))
	}
}
...
func (db *Database) commit(hash types.Hash, batch store.Batch) error {
	// If the node does not exist, it's a previously committed node
